...
public class Offset {
    ...
    public static void getBackground(TextComponent.Builder component, int target) {
        List<Integer> res = findCombination(0, 0, target);
        if (res == null) {
            return;
        }
        Chars[] bg = {Chars.BG_1, Chars.BG_2, Chars.BG_4, Chars.BG_8, Chars.BG_16, Chars.BG_32, Chars.BG_64, Chars.BG_128};
        pushHorizontalOffset(component, -target - 6);
        component.append(Component.text(Chars.BG_START.getCharacter()).style((style) -> {
            style.font(Chars.BG_START.getFont().getKey());
            style.color(TextColor.color(253, 254, 254));
        }));
        pushHorizontalOffset(component, -1);
        for (Integer re : res) {
            Chars chars = bg[re];
            component.append(Component.text(chars.getCharacter()).style((style) -> {
                style.font(chars.getFont().getKey());
                style.color(TextColor.color(253, 254, 254));
            }));
        }
        pushHorizontalOffset(component, -1);
        component.append(Component.text(Chars.BG_END.getCharacter()).style((style) -> {
            style.font(Chars.BG_END.getFont().getKey());
            style.color(TextColor.color(253, 254, 254));
        }));
