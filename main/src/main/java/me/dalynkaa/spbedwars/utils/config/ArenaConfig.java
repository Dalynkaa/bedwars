...
public class ArenaConfig {
    ...
    public File getArenaConfigFolder() {
        return this.customConfigFile.getParentFile();
    }
    ...
    public List<String> getAllMusic() {
        ...
        File musicFolder = new File(getArenaConfigFolder(), "music");
        if (musicFolder.exists()) {
            for (String musicFile : musicFolder.list()) {
                File file = new File(musicFolder, musicFile);
                music.add(file.getAbsolutePath());
            }
        }
        Logger.debug("Music: " + music.toString());
        return music;
    }
