...
public class BedStatusHud extends AbstractBossBarHud {
    ...
    public RenderStatus render(UUID player) {
        ...
        Offset.pushHorizontalOffset(bedHud, -65);
        mainBgSize += -70;
    ...
    private void drawBedStatus(TextComponent.Builder string, GameTeam team) {
        ...
        if (team.hasBed()) {
            string.append(Component.text(StatusChars.YES_CHECK.getCharacter()).style((style) -> {
                style.font(StatusChars.BED_RED.getFont().getKey());
            }));
            bgSize += StatusChars.YES_CHECK.getWidht();
        } else {
            if (team.getActivePlayers().isEmpty()) {
                string.append(Component.text(StatusChars.NO_CROSS.getCharacter()).style((style) -> {
                    style.font(StatusChars.NO_CROSS.getFont().getKey());
                }));
                bgSize += StatusChars.NO_CROSS.getWidht();
            } else {
                bgSize += StatusChars.parceNumber(string, team.getActivePlayers().size());
                mainBgSize += 3;
            }
        }
        mainBgSize += bgSize;
        Offset.pushHorizontalOffset(string, -6);
