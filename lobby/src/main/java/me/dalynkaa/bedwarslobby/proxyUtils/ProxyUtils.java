...
import me.dalynkaa.bedwarslobby.proxyUtils.data.registrators.game.GameCreationRegistrator;
import me.dalynkaa.bedwarslobby.proxyUtils.data.registrators.game.GameDeleteRegistrator;
...
public class ProxyUtils {
    ...
    public void requestGameCreation(UUID arenaId, UUID serverId, BPlayer bPlayer, ArenaTypes arenaType) {
        Jedis jedis = redisConnection.getJedis();
        GameCreationRegistrator gameCreationRegistrator = new GameCreationRegistrator(serverId, arenaType, bPlayer.getUuid(), arenaId);
        jedis.publish(Channels.GAME_CREATION_REQUEST.getChannel(), gameCreationRegistrator.toJson());
        returnRes(jedis);
    }

    public void requestGameDelete(UUID gameId, UUID serverId) {
        Jedis jedis = redisConnection.getJedis();
        jedis.publish(Channels.GAME_DELETE_REQUEST.getChannel(), new GameDeleteRegistrator(serverId, gameId).toJson());
        returnRes(jedis);
    }
    ...
    private class MyJedisSubscriber extends JedisPubSub {
        ...
        public void onMessage(String channel, String message) {
            ...
            if (channel.equals(Channels.SERVER_REGISTRATION.getChannel())) {
                ...
                ServerRegistrator serverRegistrator = ServerRegistrator.getServerByName(serverName);
                SPBedWarsLobby.getInstance().removeGameCreationServer(serverRegistrator.getServerId());
                if (bPlayer != null) {
                    bPlayer.sendServer(serverRegistrator.getServerName());
                }
            }
        }
    }
