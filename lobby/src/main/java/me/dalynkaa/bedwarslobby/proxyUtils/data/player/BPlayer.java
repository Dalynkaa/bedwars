...
//    }
    ...
    public void connectToGame(ArenaTypes type, ServerType serverType) {
        ServerRegistrator server = ServerRegistrator.getFirstServerByType(serverType);
        if (type == null) {
        ...
        if (game == null) {
            ...
            sendMessage("Создание новой игры", MessageType.NORMAL);
            server.createGame(ArenaRegistrator.selectRandomArena(), this, type);
            return;
        }
        joinGame(game);
    }
